...
if(XGETTEXT_FOUND)
  macro(gettext_create_pot potfile)
    cmake_parse_arguments(ARGS "" "PACKAGE;VERSION" "OPTIONS;SRCFILES;GLADEFILES" ${ARGN})
    ...
    if(ARGS_SRCFILES OR ARGS_GLADEFILES)
      ...
      foreach(globexpr ${ARGS_SRCFILES})
        ...
        foreach(tmpsrcfile ${tmpsrcfiles})
          file(RELATIVE_PATH relFile "${CMAKE_CURRENT_SOURCE_DIR}" "${tmpsrcfile}")
          get_filename_component(absFile "${tmpsrcfile}" ABSOLUTE)
      ...
      foreach(globexpr ${ARGS_GLADEFILES})
        foreach(tmpgladefile ${globexpr})
          file(RELATIVE_PATH relFile "${CMAKE_CURRENT_SOURCE_DIR}" "${tmpgladefile}")
          get_filename_component(absFile "${tmpgladefile}" ABSOLUTE)
