...
public class ProjectBuilder {
    ...
    public bool build_project() {
        ...
        if (!builddir.query_exists() && !builddir.make_directory())
            ...
            stderr.printf (_("Could not create directory '%s': %s\n"), buildpath, e.message);
        }
        ...
        if (project.buildsystem == "valama") {
            ...
            try {
                ...
                stderr.printf (_("Could not spawn subprocess: %s\n"), e.message);
            }
            ...
            chn.add_watch (IOCondition.IN | IOCondition.HUP, (source) => {
                ...
                try {
                    ...
                    stderr.printf (_("Could not convert all characters: %s\n"), e.message);
                } catch (GLib.IOChannelError e) {
                    stderr.printf (_("IOChannel operation failed: %s\n"), e.message);
                }
    ...
    public void launch() {
        ...
        if (!exefile.query_exists()) {
            if (build_project()) {
                ...
                if (!exefile.query_exists()) {
                    stderr.printf (_("Could not launch application: %s\n"), filename_abs);
